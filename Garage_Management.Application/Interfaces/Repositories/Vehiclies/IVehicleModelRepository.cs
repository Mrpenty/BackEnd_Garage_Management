...
namespace Garage_Management.Application.Interfaces.Repositories.Vehiclies
{
    ...
    public interface IVehicleModelRepository: IBaseRepository<VehicleModel>
    {
        ...
        Task<PagedResult<VehicleModel>> GetPagedAsync (int page, int pageSize, string? keyword = null, CancellationToken ct = default);

        /// Author: KhanhDV
        /// Created Date: 30-4-2026
        /// <summary>
        /// Lấy danh sách model thuộc về 1 brand, có phân trang + search theo tên.
        /// </summary>
        Task<PagedResult<VehicleModel>> GetByBrandIdAsync(int brandId, int page, int pageSize, string? keyword = null, CancellationToken ct = default);
